...
module metro =
    ...
    let cdf dist x =
        ...
        | Uniform(mu, sigma) ->  // 均匀分布CDF / Uniform distribution CDF
            if x < mu - sqrt(3.0) * sigma then 0.0 
            elif x > mu + sqrt(3.0) * sigma then 1.0 
            else (x - (mu - sqrt(3.0) * sigma)) / (2.0 * sqrt(3.0) * sigma)
        ...
        | Triangular(min, mode, max) ->  // 三角分布CDF / Triangular distribution CDF
            ...
            elif x < mode then (x - min) * (x - min) / ((max - min) * (mode - min)) 
            else 1.0 - (max - x) * (max - x) / ((max - min) * (max - mode))
        ...
        | Rayleigh(sigma) ->  // 瑞利分布CDF / Rayleigh distribution CDF
            if x < 0.0 then 0.0 else 1.0 - exp(-x * x / (2.0 * sigma * sigma))
        | LogNormal(mu, sigma) ->  // 对数正态分布CDF / Log-normal distribution CDF
            if x <= 0.0 then 0.0 
            else let z = (log(x) - mu) / sigma in 0.5 * (1.0 + erf(z / sqrt(2.0)))
    ...
    let pdf dist x =
        ...
        | Uniform(mu, sigma) ->  // 均匀分布PDF / Uniform distribution PDF
            if x < mu - sqrt(3.0) * sigma || x > mu + sqrt(3.0) * sigma then 0.0 
            else 1.0 / (2.0 * sqrt(3.0) * sigma)
        ...
        | Triangular(min, mode, max) ->  // 三角分布PDF / Triangular distribution PDF
            ...
            elif x < mode then 2.0 * (x - min) / ((max - min) * (mode - min)) 
        ...
        | Rayleigh(sigma) ->  // 瑞利分布PDF / Rayleigh distribution PDF
            if x < 0.0 then 0.0 else (x / (sigma * sigma)) * exp(-x * x / (2.0 * sigma * sigma))
        | LogNormal(mu, sigma) ->  // 对数正态分布PDF / Log-normal distribution PDF
            if x <= 0.0 then 0.0 
            else (1.0 / (x * sigma * sqrt(2.0 * MathHelpers.pi))) * exp(-0.5 * ((log(x) - mu) / sigma) ** 2.0)
        ...
        | Sample(samples) ->   // 样本分布PDF (使用核密度估计) / Sample distribution PDF (using kernel density estimation)
            ...
            if n = 0.0 then failwith "Sample distribution has no samples"
                ...
                let kernel u = (1.0 / sqrt(2.0 * MathHelpers.pi)) * exp(-0.5 * u * u)  // 高斯核 / Gaussian kernel
        ...
        | Bootstrap(n, samples) ->  // Bootstrap抽样PDF / Bootstrap sampling PDF
            let bootstrapSamples = BootstrapSamples(n, samples).Sample()
            if bootstrapSamples.Length = 0 then failwith "Bootstrap distribution has no samples"
            else
                let sampleStdev = sqrt(Array.average (Array.map (fun x -> let mean = Array.average bootstrapSamples in (x - mean) ** 2.0) bootstrapSamples))
                let bandwidth = 1.06 * sampleStdev * (float bootstrapSamples.Length ** (-1.0 / 5.0)) // Silverman经验法则 / Silverman's rule of thumb
                let kernel u = (1.0 / sqrt(2.0 * MathHelpers.pi)) * exp(-0.5 * u * u)
                let density = Array.sum (Array.map (fun v -> kernel ((x - v) / bandwidth)) bootstrapSamples)
    ...
    let invPdf dist p =
        ...
        | Uniform(mu, sigma) ->  // 均匀分布逆PDF / Uniform distribution inverse PDF
            ...
            if abs(p - pdfValue) < 1e-10 then mu // 在均值处唯一解 / Only one solution at the mean
        ...
        | Normal(a, b) ->  // 正态分布逆PDF / Normal distribution inverse PDF
            ...
            elif abs(p - maxPdf) < 1e-10 then mu // 在峰值处 / At the peak
            else
        ...
        | Triangular(min, mode, max) ->  // 三角分布逆PDF / Triangular distribution inverse PDF
            ...
            elif abs(p - maxPdf) < 1e-10 then mode // 在众数处 / At the mode
            else
            ...
            if n = 0.0 then failwith "Bootstrap distribution has no samples"    
                ...
                let rec binarySearch low high =
                    if high - low < 1e-6 then (low + high) / 2.0
                        ...
                        let mid = (low + high) / 2.0
                        let density = Array.sum (Array.map (fun v -> kernel ((mid - v) / bandwidth)) bootstrapSamples) / (n * bandwidth)
                        if density < p then binarySearch mid high
                        else binarySearch low mid
    ...
    let average values = (sum values) / (Exact (float (List.length values)))  // 平均值 / Average
